...
def GetComCoord(atom_list):
    ...
    '''
    # atomic mass
    mass_dict = {'C':12.010,'H':1.000,'N':14.010,'F':0.000,'S':32.060,'O':16.000,'M':0.000, 'X':28.000}
    mass_sum = 0
    x_mass_sum = 0
    y_mass_sum = 0
    z_mass_sum = 0
...
def RunGmx(run_init, em_mdp_name, md_mdp_name,version=4):
    '''
    ...
    os.system('%s mdrun -v -deffnm em_%s -nt 6 >& /dev/null' % (version1, run_init))
    ...
    os.system('%s mdrun -v -deffnm md_%s -nt 6 >& /dev/null' % (version1, run_init))
...
############################################################################################
def main(top_name, gro_name, em_mdp_name, md_mdp_name, mut_resi_name,
     candi_resi_name, sub_resi_name, switch_num, reactive_site,run_init):
    # for linux system
    #os.system('rm -rf run_backwards_* step* em_* md_* >& /dev/null')
    ...
    #run_number = 2
    if run_init != 1:
        copyfile("./run_backwards_%s/step_%s.gro" % ((run_init - 1),(run_init - 1)), "step_%s.gro" % (run_init - 1))
        copyfile("./run_backwards_%s/step_%s.top" % ((run_init - 1),(run_init - 1)), "step_%s.top" % (run_init - 1))
    while run_init <= run_number:
        ...
        if run_init == 1:
            ...
            os.system('rm -rf run_backwards_*  em_* md_* >& /dev/null')
        ...
        for info in totalmoles:
            ...
            if info[0][0] != sub_resi_name and info[0][0] != mut_resi_name:
                other_mol_list.append(info)
        #print(other_mol_list)
        ...
        #print(len(after_mut_mol_list))
        #print(random_list)
        # The mol list after mutation
        remain_mut_list = []
        ...
        for info in remain_mut_list:
            ...
            for info1 in reactive_site:
                dele_atom.append(info[info1 - 1 ])
                reactive_site1.append(info1 - 1)
            dele_mol_list = [info[i] for i in range(0, len(info), 1) if i not in reactive_site1]
            #print(dele_mol_list)
            for info3 in dele_mol_list:
                #print(info3)
                info3[0] = candi_resi_name
                tmp.append(info3)
            new_remain_mut_list.append(tmp)
            #print(new_remain_mut_list)
            #print(dele_atom)
            dele_com = GetComCoord(dele_atom)
            #print(dele_com)
            tmp_distace = []
            for info2 in sub_coord_list:
                dist = GetComDist(dele_com, info2)
        ...
        os.system('mkdir run_backwards_%s >& /dev/null' % run_init)
        os.system('cp step_%s* ./run_backwards_%s >& /dev/null' % (run_init, run_init))
        os.system('mv em_%s* ./run_backwards_%s >& /dev/null' % (run_init, run_init))
        os.system('cp md_%s* ./run_backwards_%s >& /dev/null' % (run_init, run_init))
        os.system("rm step_%d* >& /dev/null" % (run_init - 1))
        os.system('rm ./#*# >& /dev/null')
        run_init += 1

    os.system("mv md_%s.gro Final.gro >& /dev/null" % (run_init - 1))
...
if __name__ == '__main__':
    ...
    main(top_name, gro_name, em_mdp_name, md_mdp_name, mut_resi_name,
         candi_resi_name, sub_resi_name, switch_num, reactive_site,run_init)
## The End
